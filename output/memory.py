...
class RegionSetCollectionField(rsd_lib_base.ReferenceableMemberField):
    ...
    offset_mi_b = base.Field("OffsetMiB", adapter=rsd_lib_utils.num_or_none)
    """Offset with in the Memory that corresponds to the starting of this
       memory region in MiB
    """

    size_mi_b = base.Field("SizeMiB", adapter=rsd_lib_utils.num_or_none)
...
class Memory(rsd_lib_base.ResourceBase):
    ...
    capacity_mi_b = base.Field(
        "CapacityMiB", adapter=rsd_lib_utils.num_or_none
    )
    ...
    allowed_speeds_m_hz = base.Field("AllowedSpeedsMHz")
